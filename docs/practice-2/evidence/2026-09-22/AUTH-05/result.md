...
```sql
UPDATE cards SET status='EXPIRED', expiry_date=to_char((CURRENT_TIMESTAMP AT TIME ZONE 'UTC') + interval '1 month', 'MMYY') WHERE pan='4000004068007446';
```
...
```sh
curl -i -X GET http://localhost:8081/api/cards/4000004068007446
```
