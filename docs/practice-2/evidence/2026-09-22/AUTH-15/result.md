...
Дефекты: [BUG-P2-004](../../../test-design.md#bug-p2-004), [BUG-P2-005](../../../test-design.md#bug-p2-005).
...
- Прокси передаёт GET карты в CMS и возвращает HTTP 503 только для reserve этой карты.
...
```json
{
  "id": "542767f9-edd0-4cd7-b25f-2d82313422ad",
  "pan": "4000007633220675",
  "bin": "400000",
  "cardholderName": "IVAN IVANOV",
  "expiryDate": "2029-09",
  "status": "ACTIVE",
...
```sql
UPDATE cards SET status='ACTIVE', expiry_date=to_char((CURRENT_TIMESTAMP AT TIME ZONE 'UTC') + interval '1 month', 'MMYY') WHERE pan='4000007633220675';
```
...
```sh
curl -i -X GET http://localhost:8081/api/cards/4000007633220675
```
...
```text
GET /api/cards/4000007633220675 → 200
POST /api/cards/4000007633220675/reserve → 503
```
...
- [ ] Использование лимитов соответствует результату операции.
