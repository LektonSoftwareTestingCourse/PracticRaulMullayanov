...
```json
{
  "id": "e5c5b934-0e72-49b5-af64-654b05aba394",
  "pan": "4000004887211872",
  "bin": "400000",
  "cardholderName": "IVAN IVANOV",
  "expiryDate": "2029-09",
  "status": "ACTIVE",
...
```sql
UPDATE cards SET status='ACTIVE', expiry_date=to_char((CURRENT_TIMESTAMP AT TIME ZONE 'UTC') + interval '-1 month', 'MMYY') WHERE pan='4000004887211872';
INSERT INTO limit_usage(id,pan,usage_date,daily_amount,monthly_amount) VALUES(gen_random_uuid(),'4000004887211872',(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date - 1,300000,300000);
```
...
```sh
curl -i -X GET http://localhost:8081/api/cards/4000004887211872
```
...
FROM cards WHERE pan = '4000004887211872';

SELECT usage_date - (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date AS day_offset,
       daily_amount, monthly_amount
FROM limit_usage WHERE pan = '4000004887211872'
ORDER BY usage_date;

SELECT rrn, reservation_amount, status
FROM reservations WHERE pan = '4000004887211872';
```

| Состояние | Статус | Срок MMYY | B | DL | ML | D | M | Резервирований |
|---|---|---|---:|---:|---:|---:|---:|---:|
| До | ACTIVE | 0826 | 200000 | 200000 | 500000 | 0 | 300000 | 0 |
| После | ACTIVE | 0826 | 200000 | 200000 | 500000 | 0 | 300000 | 0 |
