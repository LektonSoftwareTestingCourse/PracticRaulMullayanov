...
```sql
UPDATE cards SET status='EXPIRED', expiry_date=to_char((CURRENT_TIMESTAMP AT TIME ZONE 'UTC') + interval '-1 month', 'MMYY') WHERE pan='4000005975010456';
INSERT INTO limit_usage(id,pan,usage_date,daily_amount,monthly_amount) VALUES(gen_random_uuid(),'4000005975010456',(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date - 1,300000,300000);
```
...
```sh
curl -i -X GET http://localhost:8081/api/cards/4000005975010456
```
...
FROM cards WHERE pan = '4000005975010456';

SELECT usage_date - (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date AS day_offset,
       daily_amount, monthly_amount
FROM limit_usage WHERE pan = '4000005975010456'
ORDER BY usage_date;

SELECT rrn, reservation_amount, status
FROM reservations WHERE pan = '4000005975010456';
```

| Состояние | Статус | Срок MMYY | B | DL | ML | D | M | Резервирований |
|---|---|---|---:|---:|---:|---:|---:|---:|
| До | EXPIRED | 0826 | 100000 | 100000 | 500000 | 0 | 300000 | 0 |
| После | EXPIRED | 0826 | 100000 | 100000 | 500000 | 0 | 300000 | 0 |
