...
```json
{
  "id": "4e2dc8dc-04bf-457f-851b-33c2a3d6219c",
  "pan": "4000006315580588",
  "bin": "400000",
  "cardholderName": "IVAN IVANOV",
  "expiryDate": "2029-09",
  "status": "ACTIVE",
...
```sql
UPDATE cards SET status='ACTIVE', expiry_date=to_char((CURRENT_TIMESTAMP AT TIME ZONE 'UTC') + interval '0 month', 'MMYY') WHERE pan='4000006315580588';
```
...
```sh
curl -i -X GET http://localhost:8081/api/cards/4000006315580588
```
...
FROM cards WHERE pan = '4000006315580588';

SELECT usage_date - (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date AS day_offset,
       daily_amount, monthly_amount
FROM limit_usage WHERE pan = '4000006315580588'
ORDER BY usage_date;

SELECT rrn, reservation_amount, status
FROM reservations WHERE pan = '4000006315580588';
```

| Состояние | Статус | Срок MMYY | B | DL | ML | D | M | Резервирований |
|---|---|---|---:|---:|---:|---:|---:|---:|
| До | ACTIVE | 0926 | 100000 | 150000 | 300000 | 0 | 0 | 0 |
| После | ACTIVE | 0926 | 9997 | 150000 | 300000 | 90003 | 90003 | 1 |
