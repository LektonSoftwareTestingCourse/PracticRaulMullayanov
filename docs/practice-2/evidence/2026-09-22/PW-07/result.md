...
```json
{
  "id": "47c0c52a-a5d6-4d17-8568-9c9bc2814b62",
  "pan": "4000006196810633",
  "bin": "400000",
  "cardholderName": "IVAN IVANOV",
  "expiryDate": "2029-09",
  "status": "ACTIVE",
...
```sql
UPDATE cards SET status='ACTIVE', expiry_date=to_char((CURRENT_TIMESTAMP AT TIME ZONE 'UTC') + interval '1 month', 'MMYY') WHERE pan='4000006196810633';
INSERT INTO limit_usage(id,pan,usage_date,daily_amount,monthly_amount) VALUES(gen_random_uuid(),'4000006196810633',(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date - 1,300000,300000);
```
...
```sh
curl -i -X GET http://localhost:8081/api/cards/4000006196810633
```
...
FROM cards WHERE pan = '4000006196810633';

SELECT usage_date - (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date AS day_offset,
       daily_amount, monthly_amount
FROM limit_usage WHERE pan = '4000006196810633'
ORDER BY usage_date;

SELECT rrn, reservation_amount, status
FROM reservations WHERE pan = '4000006196810633';
```

| Состояние | Статус | Срок MMYY | B | DL | ML | D | M | Резервирований |
|---|---|---|---:|---:|---:|---:|---:|---:|
| До | ACTIVE | 1026 | 100000 | 100000 | 400000 | 0 | 300000 | 0 |
| После | ACTIVE | 1026 | 0 | 100000 | 400000 | 100000 | 400000 | 1 |
